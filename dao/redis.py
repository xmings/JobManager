...
class JobCenterPersist(object):
    ...
    def fetch_task_by_id(self, job_id, job_batch_num, task_id):
        return pickle.loads(self.conn.hget(f"core.task", f"{job_id}-{job_batch_num}-{task_id}"))

    def fetch_task_by_job_id(self, job_id, job_batch_num):
        for key in self.conn.hgetall("core.task").keys():
            key = key.decode("utf8")
    ...
    def save_task(self, task: Task):
        self.conn.hset(f"core.task", f"{task.job_id}-{task.job_batch_num}-{task.task_id}", pickle.dumps(task))

    def fetch_job_by_id(self, job_id, job_batch_num):
        value = self.conn.hgetall(f"core.job-{job_id}-{job_batch_num}")
    ...
    def save_job(self, job: Job):
        self.conn.hset(f"core.job-{job.job_id}-{job.job_batch_num}", "job_name", job.job_name.encode("utf8"))
        self.conn.hset(f"core.job-{job.job_id}-{job.job_batch_num}", "status", job.status)
    ...
    def fetch_job_batch_num(self, job_id):
        ...
        batch_num += 1
        with lock:
            self.conn.hset("core.batchnum", job_id, batch_num)
        return batch_num

    def fetch_current_job_batch_num(self, job_id):
        with lock:
            batch_num = self.conn.hget("core.batchnum", job_id)
